...
message.channel.send("Kısa sürede 5 den fazla kayıt yaptığınız için yetkileriniz çekildi.")
        ...
        target.user.username.includes(tag) ? kadinRol.push(ekipRol) : kadinRol = kadinRol;
            await target.roles.remove(unreg).then(async x => {
                ...
                await teyit.updateOne({ userID: target.id }, { $push: { userName: `**[KADIN]:** \`${target.user.username.includes(tag) ? tag : tag2 ? tag2 : tag} ${name} | ${age}\` - [<t:${Math.floor(Date.now() / 1000)}:R>]` } }, { upsert: true }).exec();
                ...
                let embed = new MessageEmbed({
                    ...
                    description: `${target} kullanıcısı **Kadın** olarak kayıt edildi.`,
        ...
        exports.help = {
            name: 'kadın',
            description: "Kadın üyeleri kayıt eder.",
