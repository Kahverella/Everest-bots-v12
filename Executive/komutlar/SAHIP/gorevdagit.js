...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (!client.ayarlar.sahip.includes(message.author.id)) return;
      ...
      let dailyData = await puansystem.findOne({ guildID: client.ayarlar.sunucuId }) || { DailyMission: { Type: false }};
          ...
          client.channels.cache.get("936239088699314275").send(`\`\`\`fix\n${client.guilds.cache.get(client.ayarlar.sunucuId).name} ${moment(Date.now()).locale("tr").format("LLL")} tarihinde dağıtılan günlük görevler;\`\`\``);
          veri.forEach((user, index) => {
            setTimeout(async () => {
              ...
              if (user.gorev == "ses") {
                ...
                client.channels.cache.get("936239088699314275").send(`<@${user.user}> Bugün <#${VoiceRandom}> kanalında [\` ${miktarlar} dakika \`]  ses aktifliği yapman gerekiyor.`);
                ...
                client.channels.cache.get("936239088699314275").send(`<@${user.user}> Bugün <#${MessageRandom}> kanalında [\` ${miktarlar} mesaj \`] yazman gerekiyor.`);
                ...
                if (user.gorev == "taglı") {
                client.channels.cache.get("936239088699314275").send(`<@${user.user}> Bugün [\` ${miktarlar} taglı \`] üye çekmen gerekiyor.`);
                ...
                if (user.gorev == "teyit") {
                client.channels.cache.get("936239088699314275").send(`<@${user.user}> Bugün [\` ${miktarlar} kayıt \`] yapman gerekiyor.`);
                ...
                if (user.gorev == "davet") {
                client.channels.cache.get("936239088699314275").send(`<@${user.user}> Bugün [\` ${miktarlar} davet \`] yapman gerekiyor.`);
...
exports.conf = {
  aliases: ["gorevdagit", "görevdağıt"]
}
exports.help = {
    name: 'görevdagit',
    description: "Seçilen roldeki yetkililere görev verir.",
    usage: 'görevdagit',
