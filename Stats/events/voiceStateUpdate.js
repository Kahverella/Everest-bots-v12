...
client.on("voiceChannelJoin", async (member, channel) => {
  ...
  let vmuted = await vmuteInterval.findOne({ userID: member.id });
...
function addVoiceStat(id, channel, value, xp, category) {
  ...
  sunucuayar.findOne({ guildID: client.ayarlar.sunucuId }, (err, ress) => {
    if (!ress) return
    Stat.findOne({ userID: id, guildID: client.ayarlar.sunucuId }, (err, res) => {
      if (!res) {
        ...
        if (pub.includes(channel) || streamer.includes(channel)) {
          Stat.updateOne({ userID: id, guildID: client.ayarlar.sunucuId}, { $inc: { coin: 0.000005555555555555556 }}, { upsert: true }).exec();
        }
        ...
        if (ress.Etkinlik && ress.Etkinlik == true) {
          ...
          */
          if (pub.includes(channel)) {
            Stat.updateOne({ userID: id, guildID: client.ayarlar.sunucuId }, { 
              $inc: { EtkinlikPuan: Math.abs(value / (1000 * 60 * 60) * 5) }}, { upsert: true }).exec();
          }
        ...
        hanedan.findOne({userID: id, guildID:client.ayarlar.sunucuId}, (err, data) => {
          ...
          hanedan.updateOne({userID: id, guildID: client.ayarlar.sunucuId}, {
            $inc: { [`channel.${channel}`]: value,}}, { upsert: true }).exec()
      })

        let siradakilevel = res.voiceLevel * 643;
        if (siradakilevel <= res.voiceXP) { res.voiceLevel++ }
        res.save();
      };
    });
  });
