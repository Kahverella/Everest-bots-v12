...
module.exports.run = async (client, message, args, durum, kanal) => {
        ...
        if (message.guild.members.cache.some(member => conf.sahip.some(sahip => member === sahip)) || message.member.permissions.has(8) || message.author.id === message.guild.owner.id) {
            await sunucuayar.findOne({ guildID: message.guild.id }, async (err, data) => {
                if (args[1] == "yetki") {
                    ...
                    return data.JAILAuthorized = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "kck_tik")))
                }
                if (args[1] == "kanal") {
                    let select = message.mentions.channels.first();
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "kck_iptal"));
                    return data.JAILChannel = select.id, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "kck_tik")))
                }
                if (args[1] == "limit") {
                    let select = Number(args[2])
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "kck_iptal"));
                    return data.JAILLimit = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "kck_tik")))
                }
            })
    ...
    if (await client.permAyar(message.author.id, message.guild.id, "jail") || durum) {
        if (JailSorumlusu.length >= 1 && client.channels.cache.get(JailLogKanal) && JailLimit >= 1) {
            ...
            if (target.roles.cache.get(JailROL)) return message.react(client.emojis.cache.find(res => res.name === "kck_iptal"));
            ...
            if (client.ayarlar.CEZA_PUAN_SYSTEM == true) {
                ...
                if (cezalar.map(x => x.Puan).reduce((a, b) => a + b) >= 200) {
                    await jailInterval.findOne({ userID: target.id }, (err, data) => {
                        if (!data) {
                            newData = new jailInterval({ userID: target.id, jailed: true })
                            newData.save()
                        } else {
...
• Ceza Sebebi: [\`${reason}\`]
`)
            await jailInterval.findOne({ userID: target.id }, async (err, data) => {
                if (!data) {
...
exports.conf = {
    aliases: ["tempjail", "tjail", "Tempjail","temp-jail"]
}
exports.help = {
    name: 'jail'
}
