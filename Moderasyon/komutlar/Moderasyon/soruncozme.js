...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (durum) {
        ...
        if (["Başlat", "başlat"].includes(sec)) {
            ...
            stsc.updateOne({ userID: message.author.id, guildID: message.guild.id }, {
                $set: {
                    Type: true,
        ...
        if (["durdur", "bitir", "kapat"].includes(sec)) {
            let data = await stsc.findOne({ userID: message.author.id, guildID: message.guild.id }) || { Type: true };
...
2. Emoji: Çok iyiydi.\n3. Emoji: İyiydi\`\`\``))
                    ...
                    msg.react(birEmoji).then(async x => {
                        msg.react(ikiEmoji).then(async x => {
                            msg.react(ucEmoji).then(async x => {
                                ...
                                const birID = msg.createReactionCollector(bir, { max: 1, time: 30000, error: ['time'] });
                                const iki = (reaction, user) => reaction.emoji.name === ikiEmoji && data.Kisi.includes(user.id);
                                const ikiID = msg.createReactionCollector(iki, { max: 1, time: 30000, error: ['time'] });
                                const uc = (reaction, user) => reaction.emoji.name === ucEmoji && data.Kisi.includes(user.id);
                                const ucID = msg.createReactionCollector(uc, { max: 1, time: 30000, error: ['time'] });
        ...
        if (sec == "bilgi") {
            let emoji = client.emojis.cache.find(x => x.name === "kckterapi")
...
`)
            message.channel.send(embed).then(msg => {
                ...
                const indirID = msg.createReactionCollector(indir, { time: 60000 });
                indirID.on("collect", async r => {
                    await msg.reactions.removeAll();
                    await message.channel.send(table, { code: "md", split: true });
                });
            })
        }
        if (sec == "yardım" || !sec) {
