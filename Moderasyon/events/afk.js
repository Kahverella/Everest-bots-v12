...
module.exports = async message => {
    ...
    message.mentions.members.forEach(async (u) => {
      ...
      let ha = moment(userData.Time).fromNow()
      if (userData.Reason) {
        message.channel.send(new MessageEmbed().setColor("COLOR").setDescription("<@" + userData.userID + "> " + ha + " AFK moduna geçti. Sebep: " + userData.Reason + " ")).then(msg => { msg.delete({ timeout: 7000 }) })
      }
    });
