...
client.on("ready", async () => {
    ...
    setInterval(async () => {
        ...
        kontrol.forEach(async memberData => {
            ...
            if (memberData.Check >= mission.AMOUNT) {
                if (mission.MISSION == "ses") { arr2.push({ UserID: memberData.userID, Görev: "Ses", Puan: (mission.AMOUNT / (1000 * 60 * 60) * 40).toFixed(0) }) }
                if (mission.MISSION == "mesaj") { arr2.push({ UserID: memberData.userID, Görev: "Mesaj", Puan: (mission.AMOUNT * 0.2).toFixed(0) }) }
                if (mission.MISSION == "davet") { arr2.push({ UserID: memberData.userID, Görev: "Davet", Puan: (mission.AMOUNT * 14).toFixed(0) }) }
                if (mission.MISSION == "taglı") { arr2.push({ UserID: memberData.userID, Görev: "Taglı", Puan: (mission.AMOUNT * 40).toFixed(0) }) }
                if (mission.MISSION == "teyit") { arr2.push({ UserID: memberData.userID, Görev: "Teyit", Puan: (mission.AMOUNT * 3).toFixed(0) }) }
            }
        })
    ...
    setInterval(async () => {
        ...
        let dagit = []
        await dayMiss.findOne({ guildID: mainSettings.sunucuId }, async (err, res) => {
            if (!res) {
                let newData = dayMiss({ guildID: mainSettings.sunucuId, Date: Date.now() })
                await newData.save()
            } else {
                if (Date.now() - res.Date >= 1000 * 60 * 60 * 24 * 1) {
                    ...
                    kontrol.forEach(async memberData => {
                        let mission = memberData.Mission;
                        if (memberData.Check >= mission.AMOUNT) {
                            randMiss.deleteOne({ userID: memberData.userID }).exec();
                            if (mission.MISSION == "ses") { xpData.updateOne({ userID: memberData.userID }, { $inc: { currentXP: (mission.AMOUNT / (1000 * 60 * 60) * 40) } }, { upsert: true }).exec() }
                            if (mission.MISSION == "mesaj") { xpData.updateOne({ userID: memberData.userID }, { $inc: { currentXP: (mission.AMOUNT * 0.2) } }, { upsert: true }).exec() }
                            if (mission.MISSION == "davet") { xpData.updateOne({ userID: memberData.userID }, { $inc: { currentXP: (mission.AMOUNT * 14) } }, { upsert: true }).exec() }
                            if (mission.MISSION == "taglı") { xpData.updateOne({ userID: memberData.userID }, { $inc: { currentXP: (mission.AMOUNT * 40) } }, { upsert: true }).exec() }
                            if (mission.MISSION == "teyit") { xpData.updateOne({ userID: memberData.userID }, { $inc: { currentXP: (mission.AMOUNT * 3) } }, { upsert: true }).exec() }
                        }
                    })
                    setTimeout(async () => {
                        let sunucudata = await sunucuayar.findOne({});
                        client.guilds.cache.get(mainSettings.sunucuId).roles.cache.get(sunucudata.EnAltYetkiliRol).members.array().shuffle().forEach((x) => {
                            arr.shuffle()
                            let random = arr[Math.floor(Math.random() * arr.length)]
                            dagit.push({ user: x.id, gorev: random })
                        });
                        let veri = dagit;
                        ...
                        client.channels.cache.get("939810977543782490").send(`\`\`\`${client.guilds.cache.get(mainSettings.sunucuId).name} ${moment(Date.now()).locale("tr").format("LLL")} tarihinde dağıtılan günlük görevler;\`\`\``);
                        veri.forEach((user, index) => {
                            setTimeout(async () => {
                                ...
                                if (user.gorev == "ses") {
                                    ...
                                    client.channels.cache.get("939810977543782490").send(`<@${user.user}> Bugün <#${VoiceRandom}> kanalında \`${miktarlar}\` dakika ses aktifliği yapman gerekiyor.`);
                                ...
                                if (user.gorev == "mesaj") {
                                    ...
                                    client.channels.cache.get("939810977543782490").send(`<@${user.user}> Bugün <#${MessageRandom}> kanalında \`${miktarlar}\` adet mesaj yazman gerekiyor.`);
                                ...
                                if (user.gorev == "taglı") {
                                    client.channels.cache.get("939810977543782490").send(`<@${user.user}> Bugün \`${miktarlar}\` adet taglı üye çekmen gerekiyor.`);
                                ...
                                if (user.gorev == "teyit") {
                                    client.channels.cache.get("939810977543782490").send(`<@${user.user}> Bugün \`${miktarlar}\` adet teyit yapman gerekiyor.`);
                                ...
                                if (user.gorev == "davet") {
                                    client.channels.cache.get("939810977543782490").send(`<@${user.user}> Bugün \`${miktarlar}\` adet davet yapman gerekiyor.`);
...
client.on("ready", async () => {
    try {
        console.log(`[Async]: ${client.user.username} ismi ile giriş yapıldı!`);
        ...
        setInterval(() => {
            ...
            kanal.map(channel => {
                if (channel.id === mainSettings.botSesID) {
                    ...
                    client.channels.cache.get(channel.id).join().then(() => console.log("[Async] başarılı bir şekilde ses kanalına bağlandı")).catch(() => console.log("[Async] ses kanalına bağlanırken bir sorun çıktı Lütfen Yetkileri kontrol ediniz!"));
                } else return;
            });
        }, 10000);
...
client.login(mainSettings.ASYNC).catch(() => console.log("[Async] Token bozulmuş lütfen yeni bir token girmeyi dene"));
